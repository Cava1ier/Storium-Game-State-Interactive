...
export const PipIcon: React.FC<{ strength: 'strong' | 'weak' | 'empty', className?: string }> = ({ strength, className='w-4 h-4' }) => {
    const baseClasses = "rounded-sm";
    const colorClasses = {
        strong: 'bg-green-400',
        weak: 'bg-yellow-400',
        empty: 'bg-gray-600 border border-gray-500',
    };
    return <div className={`${baseClasses} ${colorClasses[strength]} ${className}`}></div>;
};
...
export const CancelIcon: React.FC<{ className?: string }> = ({ className = 'w-5 h-5' }) => (
    <svg xmlns="http://www.w3.org/2000/svg" className={className} viewBox="0 0 20 20" fill="currentColor">
